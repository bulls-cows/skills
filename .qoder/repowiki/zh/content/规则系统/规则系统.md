...
## 项目结构
规则系统以“主规则文件 + 子模块 references”的方式组织，确保规则可读、可维护、可组合。Vue2 与 Vue3 各自拥有独立的 RULE.md 与 references 目录，形成“总纲索引 + 子模块”的知识图谱。
...
## 架构总览
规则系统采用“主规则 + 子模块”的分层架构，结合元数据与配置文件，实现跨平台复用与版本治理。
...
## 依赖分析
规则系统内部依赖关系清晰：主规则文件依赖 references 子模块；Vue3 通过 metadata.json 提供结构与版本信息；配置文件决定规则在不同工具链中的可见性与生效范围。
...
## 结论
规则系统通过“主规则 + 子模块 + 元数据 + 配置”的组合，实现了跨平台、可演进、可维护的规则治理方案。Vue2 与 Vue3 的差异化设计满足不同技术栈的需求，而文档与文本规则则保障了知识表达的一致性与可执行性。建议在团队内推广该体系，结合工具链自动化校验，持续提升代码质量与开发效率。
