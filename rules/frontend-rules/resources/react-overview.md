...
- 使用 Hooks 管理状态和副作用
- **禁止**在条件、循环或嵌套函数中调用 Hooks（详见 [hooks.md](./react-hooks.md#二hooks-调用规则)）

### TypeScript 优先

- 新项目优先使用 TypeScript
- Props、State、Refs 必须显式标注类型（详见 [typescript.md](./react-typescript.md)）
...
| 组件交互与通信  | [interaction.md](./react-interaction.md)         | Props 单向流、回调、状态提升、Context、Render Props、状态管理选型 |
| 代码组织与顺序  | [order.md](./react-order.md)                     | 文件结构、组件内部顺序、Import 分组（4 组）、JSX 属性顺序         |
| Hooks 规范      | [hooks.md](./react-hooks.md)                     | Hook 调用规则、内置 Hooks                                         |
| 状态管理        | [state.md](./react-state.md)                     | useState / useReducer                                             |
