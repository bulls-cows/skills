...
| 3   | 深层可选链 `?.`     | 不推荐超过 2 层的可选链 `a?.b?.c`，可读性差              | 使用 lodash 的 `lodash.get(a, ['b', 'c'], defaultValue)` 替代 |
...
2. **try-catch 错误保底**：方法体必须包裹在 try-catch 中，catch 中使用 `console.warn(error)` 记录异常
...
| 项             | 说明                                                                                            |
| -------------- | ----------------------------------------------------------------------------------------------- |
| **未使用变量** | ESLint 已关闭未使用变量检查，开发者需自行清理无用代码                                           |
| **动态 HTML**  | 允许使用 `v-html` / `dangerouslySetInnerHTML`，但必须使用 DOMPurify 过滤内容，防范 XSS 风险     |
| **props 解构** | 允许解构 props，但会丢失响应式，值不再随 props 更新                                             |
| **等于运算符** | 项目偏好 `==`，网络请求时使用 `===`；改动等号时需提醒用户确认                                   |
| **注释检查**   | 注释相关问题默认忽略，开发者自行维护注释准确性，详见 [common-comments.md](./common-comments.md) |
| **过度封装**   | 避免过度封装，简单逻辑直接写在模板中，提高可读性                                                |
| **魔法数字**   | 避免无意义的字面量数字，如 `if (status === 3)`，需提取为常量 `const STATUS_AUDIT_PASS = 3`      |
