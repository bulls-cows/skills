...
| 类型              | 规范                               | 示例                                              | 说明                               |
| ----------------- | ---------------------------------- | ------------------------------------------------- | ---------------------------------- |
| API 函数          | `api` + 方法 + 业务含义（小驼峰）  | `apiGetUserInfo`、`apiPostLogin`、`apiDeleteUser` | 明确标识为 API 请求                |
| 事件处理函数      | `on` + 触发元素 + 事件名（小驼峰） | `onSubmitBtnClick`、`onUserNameInputChange`       | 说明触发源与事件类型               |
| 工具函数          | 动词 + 名词（小驼峰）              | `formatDate`、`getUuid`、`parseQuery`             | 明确函数作用                       |
| Vue/React Hooks   | `use` + 功能（小驼峰）             | `useUserInfo`、`useRouterQuery`、`useWindowSize`  | 符合 Hooks 规范                    |
| 计算属性/派生函数 | 名词/形容词（小驼峰）              | `fullName`、`isDisabled`、`filteredList`          | 表示值而非动作                     |
| 操作函数          | 动词 + 名词（小驼峰）              | `openDialog`、`closeModal`、`loadData`            | 明确执行动作                       |
| 函数参数          | 小驼峰 camelCase，语义清晰         | `userId`、`userInfo`、`formatter`                 | 与变量一致，禁止单字母或无意义命名 |
...
| 类型               | 说明                   | 示例                                                        | 规则                                      |
| ------------------ | ---------------------- | ----------------------------------------------------------- | ----------------------------------------- |
| Block（块）        | 独立的组件/模块        | `.card`, `.user-form`, `.menu`                              | 全小写，短横线分隔多单词，代表独立功能块  |
