...
| 内联 `style`            | 仅动态计算的单个属性         | ❌ 禁止（见 [jsx.md](./jsx.md#九样式使用)） |
...
样式文件必须以 `.module.scss`（或 `.module.css`）为后缀，导入语句归在 import 第 4 组「相对依赖」（详见 [order.md](./order.md#三import-分组排序4-组)）：
...
| 样式使用（jsx）  | [jsx.md](./jsx.md#九样式使用)         |
| 导入顺序         | [order.md](./order.md#三import-分组排序4-组) |
