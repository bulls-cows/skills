...
> 本规范涵盖 Vue2/Vue3 通用的 watch 理念：深度监听、立即执行、资源清理、与 computed 的选择策略。每个理念点并列给出 Vue2（Options API `watch` 选项）与 Vue3（`watch()` 函数）写法。框架特定差异（如 Vue3 的 `watchEffect`、Vue2 的字符串路径监听）详见各自框架文档。

---

## 一、基本规则

- **深度监听**：监听对象/数组内部变化必须开启深度监听（Vue2/Vue3 均为 `deep: true`）
- **立即执行**：初始化即需触发回调时必须声明（Vue2/Vue3 均为 `immediate: true`）
