...
## 目录命名约定（Vibe Coding 术语）

为了方便沟通和快速定位，本项目对主要目录约定以下统一称谓：

| 目录               | 称谓             | 说明                       |
| ------------------ | ---------------- | -------------------------- |
| `skills/`          | **技能目录**     | 对外发布的所有技能的根目录 |
| `skills-internal/` | **内部技能目录** | 内部技能目录，不对外发布   |
| `rules/`           | **规则目录**     | 自定义规则目录             |

使用场景示例：

- "去技能目录加个新技能" → 指 `skills/` 目录
- "规则目录里加条规范" → 指 `rules/` 目录
...
- [记忆管理规则 @.agents/rules/memory/RULE.md](./.agents/rules/memory/RULE.md)：对话启动时自动加载 MEMORY.md，手动执行 [yy-memory](./.agents/skills/yy-memory) 技能后更新记忆
