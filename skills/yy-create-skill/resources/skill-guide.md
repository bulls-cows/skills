...
9. 同一任务重复运行，输出结构稳定
10. Skill 能单独被显式调用
11. 如果有参考文档需求，已添加 `resources/` 目录
12. 如有脚本文件，遵循 snake_case 命名规范
13. 如有 evals 测试用例，格式正确

### 更新技能后检查项

1. `## 描述` 章节仍然准确反映技能用途
2. 更新后的 description 仍然准确反映技能用途
3. description 只保留触发判断所需信息，没有混入执行细节
4. 使用场景仍然明确（触发条件和不应触发场景）
5. 指令步骤仍然完整可执行，最后一步描述输出格式
6. YAML 格式正确
7. 如有参考文件，检查是否需要同步更新
