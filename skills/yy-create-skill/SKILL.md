...
- 根据用户输入确定更新范围：
  ...
  - **未明确指定，但目标技能明确**：先展示计划，默认按"补充+优化"策略处理；如当前分支需要确认，则等待用户确认后继续
...
- **同步检查辅助文件**：修改 SKILL.md 后，检查 examples/、templates/、resources/、prompts/ 中的文件是否需要同步更新。默认只修改 SKILL.md；若本次变更导致辅助文件中的步骤编号、章节名称、字段名或提示词内容与新的 SKILL.md 失配，则允许同步更新相关辅助文件
...
- 目标技能、修改范围或默认策略仍不明确时，不得直接批量创建、覆盖或改写文件；应先回到步骤 1 和步骤 2 补齐信息或确认方案
