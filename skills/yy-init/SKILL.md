...
name: yy-init
description: >
  初始化或更新项目的 AGENTS.md 文档，指导 AI 助手理解项目规范、范围和结构。
  当用户提到"初始化项目"、"创建 AGENTS.md"、"项目规范文档"时触发。
icon: 🚀
examples:
  - /yy-init
  - 初始化项目 AGENTS.md
...
初始化或更新项目的 AGENTS.md 文档，确保 AI 助手能够理解项目的规范、范围和结构。
...
- 若不存在：提示用户将创建新文件，继续执行步骤 2
- 若存在：读取现有内容，询问用户是"更新补充"还是"完全重写"，默认采用"更新补充"策略
...
✓ 包含章节: 用户特质, 范围, 改动检查, 交付格式, 项目结构, 规则引用
```
