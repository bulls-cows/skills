...
**快速通道（Fast Track）**：在委托专项技能之前，先对目标文件执行对应参考文件（如 `vue3.md`）中定义的快速通道 grep 模式匹配。快速通道发现的缺陷直接纳入审核结果，按原有严重程度分级，并在末尾标注 `[FastTrack]` 标记。委托专项技能后，将快速通道结果与技能结果合并，按 `文件路径:行号` + `问题类型` 去重排序后输出。
