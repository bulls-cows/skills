...
> 目录范围定义在 `config/file-scope.json`，当前允许处理的目录：
...
过滤出 config/file-scope.json 中允许的目录下的文件（**注意：仅处理允许目录下的文件，其他目录的文件不参与提交**）：
...
我将帮你：
1. 归纳 config/file-scope.json 中允许的目录下所有改动的文件
2. 分析改动内容
3. 生成规范的提交信息
...
- 允许的目录定义在 config/file-scope.json
