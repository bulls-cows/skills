...
description: >
  前端代码提交助手，仅在存在允许目录结构的前端项目中触发。
  归纳 src 目录下改动的文件，生成规范的提交信息并自动执行 add 和 commit 操作。
...
1. **首先检查项目是否存在允许的目录**：
   ...
   - 如果都不存在，直接告诉用户："当前项目不符合前端代码提交助手的目录要求，该技能仅支持包含上述目录的前端项目，不触发提交流程。" 并终止执行
...
- 如果过滤后没有符合条件的文件，告诉用户："当前没有需要提交的文件（仅允许提交指定目录下的改动文件），提交流程结束。" 并终止执行
