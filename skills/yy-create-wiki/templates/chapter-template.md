...
每个 Wiki 文档必须包含以下结构：标题 → `<cite>` 引用块 → `## 目录` 导航 → 10 个标准章节。

## 文档开头

### `<cite>` 引用块

每个文档标题之后，必须包含 `<cite>` 引用块，列出该文档引用的所有源文件。
...
`<cite>` 块之后，插入目录导航章节，列出该文档包含的所有标准章节锚点链接。
...
- **`<cite>` 引用块**：每个文档开头必须包含 `<cite>` 块，列出该文档引用的所有源文件
- **目录导航**：`<cite>` 块之后必须包含 `## 目录` 章节，列出标准章节锚点链接
