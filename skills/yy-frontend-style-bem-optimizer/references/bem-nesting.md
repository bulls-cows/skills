...
- **独立选择器嵌套后增加特异性**：`.block__element` 是单类选择器（0,1,0），嵌套到 `.block { &__element {} }` 后展开为 `.block .block__element`（0,2,0），特异性增加了 1。正确做法是 `.block { &__element {} }` 展开为 `.block__element`（0,1,0），大多数预处理器默认如此，但若原代码中有 `.block .block__element` 这种后代选择器则需保持原样
- **复合选择器**：`.block--modifier.block` 这样的复合选择器嵌套后可能变为 `.block .block--modifier`，特异性从 (0,2,0) 变为 (0,2,0) 但匹配规则不同，需人工确认
- **伪类嵌套**：`.block:hover` 嵌套为 `.block { &:hover {} }` 特异性不变，但 `.block__element:hover` 需确保嵌套展开为 `.block__element:hover` 而非 `.block .block__element:hover`

**安全策略**：

- 对于无法确定特异性是否一致的情况，保持扁平不嵌套，在报告中标记
- 嵌套重组后应执行编译验证——确保重组前后编译产物一致

## 特异性验证

嵌套重组后必须验证选择器特异性是否与嵌套前一致。不一致的嵌套会导致样式优先级变化，可能覆盖或被覆盖其他规则。

**验证方法**：

1. 嵌套前，记录每个选择器的特异性（如 `.block__element` = 0,1,0）
2. 嵌套后，展开 `&` 引用计算实际选择器的特异性
3. 比较两者是否一致
