...
- **最终判断为应分次提交，用户已选择一个或多个分组**：对每个选中的分组，独立执行步骤 4 至步骤 8

> 分次提交执行过程中（即步骤 6 之后）的循环控制规则在步骤 6 末尾的"分次提交循环控制"小节中说明。
...
用户确认后，按顺序执行（不可并行）。先清空暂存区，再按步骤 4 的决策重新暂存，确保最终提交内容与步骤 4 的决策严格一致：
