...
- 初步判断为应拆分时，向用户展示分组方案，由用户再次确认是否分次提交以及选择哪些分组，用户确认结果即为最终判断
...
#### "初步判断"为应拆分成多次提交时的具体交互

将改动按功能/模块/逻辑单元分组，向用户展示分组方案供其确认，允许同时选择多个分组：
...
- **最终判断为无需分次提交**（初步判断即无需分次，或用户在分组方案中选择"无需分次提交"）：按单次提交流程继续后续步骤；若来自用户在分组场景下的合并选择，在确认信息中说明多个逻辑单元将合并提交
- **最终判断为应分次提交，用户已选择一个或多个分组**：对每个选中的分组，独立执行步骤 4 至步骤 8；每组开始前先执行 `git reset HEAD` 清空暂存区，再 `git add` 当前组的文件，确保组间暂存区隔离
- **用户未选择任何分组（含取消操作）**：告知用户未选择任何提交分组，流程结束
