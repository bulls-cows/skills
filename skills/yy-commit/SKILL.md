...
- 用户选定候选后进入步骤 5 的最终确认；用户选择"重新生成"时回到本步骤重新生成候选
- **快速提交模式（步骤 5 触发条件命中）**：直接生成 1 个候选并进入步骤 5 展示，跳过候选展示与选择交互
...
**敏感文件命名特征清单**（仅对新增文件标注 ⚠️ 提示，已追踪文件的修改不提醒）：
...
- **选 `1`**：进入步骤 6 执行提交
- **选 `0`**：终止本次提交流程，输出"已取消"
- **选 `2`**：收集用户对提交信息的修改意见，按"修改提交信息规则"重新生成，并回到步骤 5 重新展示确认选项；此循环可多次发生
...
建议分别提交以保持提交历史清晰。本次先提交其中一组，剩余分组请再次调用本技能处理。
```
