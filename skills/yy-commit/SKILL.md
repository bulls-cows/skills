...
2. 将要暂存的文件列表
3. 新增敏感文件警告（如暂存列表中存在新增的敏感命名特征文件，详见 [resources/sensitive-file-patterns.md](./resources/sensitive-file-patterns.md)）
...
- **选 `1`**：进入步骤 5 执行提交
- **选 `0`**：终止本次提交流程，输出"已取消"
- **选 `2`**：收集用户对提交信息的修改意见，按"修改提交信息规则"重新生成，并回到步骤 4 重新展示确认选项；此循环可多次发生
