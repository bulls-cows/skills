...
- **判断为应分次提交**：将改动按功能/模块/逻辑单元分组，以编号列表展示分组选项，让用户确认提交方案：
  - 每个分组一个编号，简述改动内容并列出涉及的文件，允许用户同时选择多个分组
  - 额外提供"合并为单次提交"选项，表示不拆分、一次性提交所有改动
  - **互斥规则**：若用户同时选中了分组选项和"合并为单次提交"，以"合并为单次提交"为准，忽略所有分组选项
- **用户未选择任何分组（含取消操作）**：告知用户未选择任何提交分组，流程结束
- **用户选择分组后**：对每个选中的分组，独立执行步骤 4 至步骤 8；每组开始前先执行 `git reset HEAD` 清空暂存区，再 `git add` 当前组的文件，确保组间暂存区隔离
- **用户选择"合并为单次提交"**：按单次提交流程处理所有改动，并在确认信息中说明多个逻辑单元将合并提交
...
当单次改动涉及多个维度（如同时包含功能新增和重构），或改动意图可以从不同角度描述时，以编号列表提供 2 个候选提交信息供用户选择，帮助用户找到最准确的表达。用户选择后进入步骤 6 的最终确认。简单改动（意图明确、变更单一）只需生成 1 个即可。
