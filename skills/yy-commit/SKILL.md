...
- **全部变更为默认不暂存文件**：当所有有实际内容变更的文件均属于默认不暂存类型时，自动暂存这些文件；若包含敏感文件，只在展示阶段给出警告提醒
- **用户未明确授权且只有部分文件为默认不暂存**：从暂存列表中排除所有默认不暂存的文件，并在展示阶段明确告知排除原因
...
3. 敏感文件警告（如暂存或排除列表中存在敏感文件）
4. 关键变更摘要（从 diff 提取的主要改动）
5. 生成的完整提交信息（默认含 body）

**前端变更日志检查：**

在展示确认信息的同时，检查暂存文件中是否包含前端文件（`.vue`、`.jsx`、`.tsx`、`.js`、`.ts`、`.css`、`.scss`、`.less` 等）。如果包含，在确认信息末尾追加提示：
...
如果用户在请求中使用了以下关键词，则跳过确认步骤（同时跳过前端变更日志检查）：
