...
- **判断为应分次提交**：向用户说明拆分建议，给出拆分方案（按功能/模块/逻辑单元分组），等待用户确认后再逐组执行暂存和提交流程
...
- **全部变更为默认不暂存文件**：当所有有实际内容变更的文件均属于"默认不暂存"类型时，自动暂存这些文件，不再逐项确认。用户主动调用提交技能即表明提交意图，无需额外授权。
...
- 用户选择生成变更日志时，先执行 `yy-frontend-change-log` 技能，完成后再回到本提交流程继续执行
- 用户选择直接提交或使用快速提交模式时，跳过变更日志生成，直接执行提交
