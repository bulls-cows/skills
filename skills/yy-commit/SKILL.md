...
- 以当前 `git status`、`git diff` 和 `git diff --staged` 为提交范围的完整事实来源；已暂存、未暂存和未跟踪的改动均默认属于用户
- 对话上下文只用于理解改动意图和生成提交信息，不得据此排除、覆盖或还原工作区中未在对话里提及的改动
- 无法将某项改动对应到对话需求时，展示该项并询问用户如何分组；不得将其认定为多余改动或跳过重新暂存
- 最终选定的每个文件均按当前完整内容暂存；不得仅保留其中与对话需求相关的代码块，或自动还原其余代码块
...
1. **暂存区副作用提示**（仅当步骤 1 检测到 `git diff --staged` 非空，且本次提交的暂存范围与已有暂存内容不一致时）：明确告知"执行提交前会先 `git reset HEAD` 重建暂存区，再按以下文件的当前完整内容重新暂存；该操作不会还原工作区文件"，并在用户确认前等待用户明确同意
2. 将要暂存的文件列表，确认其中包含用户要求提交的全部手动改动
...
```bash
# 1. 清空暂存区，重建最终选定的暂存内容；不会修改工作区文件
git reset HEAD

# 2. 按当前完整内容暂存选定的文件，不得根据对话上下文筛除代码块
git add <file1> <file2> ...
...
- 还原用户改动：不因改动未在对话中提及而使用 `git restore`、`git checkout --`、`git clean` 或补丁覆盖工作区文件
