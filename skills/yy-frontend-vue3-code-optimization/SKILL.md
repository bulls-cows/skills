...
| ------- | ------------ | --------- | -------------------------------------------------------------------------------- | -------------- |
| T01     | 业务逻辑梳理 | 🟢 零风险 | 仅 .vue，生成业务说明 JSDoc                                                      | 文件子代理执行 |
| T02     | 注释增强     | 🟢 零风险 | 模板/脚本/样式注释，只增不改                                                     | 文件子代理执行 |
| T03     | 代码风格清洗 | 🟡 中风险 | 导入排序(4组)、`<script setup>`结构、模板属性顺序、组件 name 属性                | 文件子代理执行 |
| T04     | CSS/BEM 规范 | 🟡 中风险 | 类名转为 BEM 格式，scoped 同步修改                                               | 文件子代理执行 |
| T05     | 语义化命名   | 🟡 中风险 | API/事件/常量/Hooks 命名规范                                                     | 文件子代理执行 |
...
- **风险分层**：零/中风险任务由文件子代理自主处理，高风险任务提升至主代理逐项确认
- **并行执行**：多个文件可同时并行处理，显著提升整体效率
- **故障隔离**：单个文件处理失败不影响其他文件的优化
...
│ 的所有零/中风险  │  │ 的所有零/中风险  │  │ 的所有零/中风险  │
│ 任务（T01-T05） │  │ 任务（T01-T05） │  │ 任务（T02-T05） │
└─────────────────┘  └─────────────────┘  └─────────────────┘
...
5. 任务执行（用户确认后执行）
    ├── 零风险任务（T01, T02）→ 按文件创建子代理，各文件并行自动执行
    ├── 中风险任务（T03, T04, T05）→ 按文件创建子代理，各文件并行执行
...
| 阶段   | 任务          | 执行条件              | 并行度         |
| ------ | ------------- | --------------------- | -------------- |
| 阶段一 | T01, T02      | 🟢 用户确认后自动执行 | 按文件并行处理 |
| 阶段二 | T03, T04, T05 | 🟡 用户确认后执行     | 按文件并行处理 |
| 阶段三 | T06, T07      | 🔴 逐项确认后执行     | 主代理串行执行 |
...
| 文件         | 执行任务                | 状态    |
| ------------ | ----------------------- | ------- |
| UserList.vue | T01, T02, T03, T04, T05 | ✅ 完成 |
| UserCard.vue | T01, T02, T03           | ✅ 完成 |
| user.ts      | T02, T03, T05           | ✅ 完成 |
| common.scss  | T03, T04                | ⏭️ 跳过 |
