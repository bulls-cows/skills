...
- 用户指定文件或文件夹进行优化（.vue/.js/.jsx/.ts/.tsx/.css/.scss/.less）
- 用户未指定文件时，自动优化当前 git 变动文件（含暂存区）
- 用户要求统一 `<script setup>` 结构、导入排序、格式化代码
- 用户要求进行 Hooks 抽离、语义化命名重构、异步代码优化、注释增强
- 用户要求 TypeScript 类型注解优化
...
1. **用户指定**：递归收集指定文件/文件夹内的 `.vue`、`.js`、`.jsx`、`.ts`、`.tsx`、`.css`、`.scss`、`.less` 文件。
2. **未指定**：执行 `git diff --name-only HEAD` 和 `git diff --cached --name-only`，合并去重后过滤出支持的文件类型。
3. **无匹配文件**：回复「当前没有需要优化的改动文件（支持 .vue、.js、.jsx、.ts、.tsx、.css、.scss、.less）。你可以指定文件或文件夹让我优化。」并终止。
...
| T02     | `.vue` `.js` `.jsx` `.ts` `.tsx` | 代码风格与格式清洗 | 统一缩进、引号、分号、导入排序(11组)、`<script setup>`结构排序、箭头函数写法 | 🟡 中风险    |
...
API/事件重命名、常量规范、布尔值前缀规范、Hooks 命名规范。跨文件引用需确认。
...
| 禁止使用 any 类型 | TypeScript 中参数、返回值、变量必须明确类型，用 `unknown` 或具体类型 |
| 绝不修改业务逻辑              | 组件拆分须用户确认    |
| 不生成新组件                  |                     |
...
4. **Hooks 抽离**：可复用逻辑超过 30 行或跨 2+ 组件使用时，抽离为 Hook（全局存放在 `@src/hooks/`，局部直接在组件同级目录新建文件）
...
9. **函数写法**：优先使用 `const 函数名 = () => {}` 箭头函数写法，避免 `function` 声明
...
| **TypeScript/JSX/TSX 支持** | 参数、返回值、变量必须明确类型，禁止 `any`（用 `unknown` 或具体类型） |
