...
| T06     | 逻辑深度优化 | 🔴 高风险 | async/await、Hooks抽离、**reactive转ref（尽可能少用reactive）**、Props/Emits增强 | 子代理 T06 |
...
└─────────────────────────────────────────────────────────────────────┘
         ...
         ▼                    ▼                    ▼
┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
│  零风险流水线    │  │  中风险流水线    │  │  高风险流水线    │
│  (自动执行)       │  │  (确认后执行)    │  │  (确认后执行)    │
├─────────────────┤  ├─────────────────┤  ├─────────────────┤
│ 子代理 T01       │  │ 子代理 T03       │  │ 子代理 T06       │
│ 业务逻辑梳理      │  │ 代码风格清洗     │  │ 逻辑深度优化     │
│ (可并行处理多文件) │  │ (可并行处理多文件)│  │ (可并行处理多文件)│
├─────────────────┤  ├─────────────────┤  └─────────────────┘
│ 子代理 T02       │  │ 子代理 T04       │
│ 注释增强         │  │ CSS/BEM 规范     │
│ (可并行处理多文件) │  │ (可并行处理多文件)│
...
| 文件类型            | 匹配任务                                       |
| ------------------- | ---------------------------------------------- |
| `.vue`              | T01, T02, T03, T04, T05, T06                  |
| `.js/.jsx/.ts/.tsx` | T02, T03, T05, T06                            |
