...
| T02 | [filename] | 代码风格与格式清洗 | 统一缩进/引号/分号/导入排序(11组)/`<script setup>`结构排序/箭头函数写法 | 🟡 中风险 |
...
- 返回值必须使用对象（推荐 `toRefs` 解构），**禁止直接返回 reactive 对象**
- 禁止将 Hooks 挂载到响应式数据上
...
9. 性能优化：路由和大组件使用 `defineAsyncComponent`，合理使用 `<KeepAlive>`
