...
当用户提到以下内容**并且项目存在 src 目录（且至少包含 src/api、src/components、src/views 等允许目录中的一个）**时，使用此 skill：
...
1. **首先检查项目是否存在允许的目录**：
   - 检查当前项目是否存在 `src/api`、`src/components`、`src/constants`、`src/views`、`src/pages` 中至少一个目录
   - 如果都不存在，直接告诉用户："当前项目不符合前端代码审核助手的目录要求，该技能仅支持包含上述目录的前端项目，不触发审核流程。" 并终止执行
...
3. 过滤出允许的目录下的文件（**注意：仅处理允许目录下的文件，其他目录的文件不参与审核**）
...
我将帮助你审核允许的目录下所有改动的文件：

1. 检查是否存在 bug 或潜在问题
2. 检查是否存在安全漏洞
3. 检查代码最佳实践

注意：允许的目录定义在 SKILL.md 头部的 metadata.allowed_directories
