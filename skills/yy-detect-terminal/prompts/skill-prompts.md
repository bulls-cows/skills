...
- **边界**：只处理终端能力识别与记录维护，不执行 lint、测试、构建、部署、安装依赖等业务命令，不读取或输出敏感内容；除目标项目位于 Git 仓库且仓库根目录已有 `.gitignore` 时可追加 `TERMINAL.LOCAL.md` 对应忽略项外，不修改其他文件
...
读取方式：优先使用 Agent 原生文件读取能力；若原生读取不可用，直接使用 `node` 进程读取文件内容，不通过 shell 包装；只有原生读取与 `node` 读取均不可用时，才按固定优先级执行最小 shell 读取探测；启动读取阶段只用于判断文件是否存在并读取内容，不代表终端能力结论。
...
验证要求：已有记录只能作为候选来源；如果当前会话只能通过低优先级 shell 启动命令，该 shell 只能作为检测入口，必须完成步骤 3 的候选枚举和启动验证后，才能确定首选 shell、备用 shell 和不可用 shell。

### 步骤 3. 识别 shell 启动入口

按固定优先级识别当前环境可调用的 shell 启动入口，优先级为 `pwsh`、`powershell.exe`、`powershell`、`bash`、`cmd.exe`。

- 必须完整枚举上述候选列表，不得因为当前命令通过低优先级 shell 成功执行就停止识别
...
- 不可用 shell 只能记录已经完成存在性判断和启动验证且失败的候选；无法验证的候选必须记录为待确认
...
- 记录首选 shell 时必须使用固定优先级的验证结果，不能使用”当前命令刚好在哪个 shell 成功执行”作为依据
...
输出检测入口、首选 shell、备用 shell、不可用 shell、待确认 shell、可用搜索工具、验证证据、文件状态、后续命令建议和异常说明。文件状态需包含 `TERMINAL.LOCAL.md` 是新建、更新还是无需修改，`.gitignore` 是已追加、无需修改、未找到还是不适用；异常说明中如有无法确认的命令能力，必须明确标记待确认。
...
- 禁止为识别命令能力而修改 `TERMINAL.LOCAL.md` 以外的文件；唯一例外是目标项目位于 Git 仓库且仓库根目录已有 `.gitignore` 时，只可追加 `TERMINAL.LOCAL.md` 对应忽略项
...
- 优先通过 Agent 原生读取或直接 `node` 进程读取已有 `TERMINAL.LOCAL.md`，再决定是否重新识别
- 已有记录和当前检测入口都只能作为候选来源，不能替代完整候选枚举
