...
description: >
  在目标项目中一键启用分层记忆管理系统，生成规则、技能和记忆库结构。
...
- `.agents/rules/` 目录（不存在则创建）
- `.agents/skills/` 目录（不存在则创建）
- `.agents/rules/memory/` 目录
- `.agents/skills/yy-memory/` 目录

**决策分支**：

- **AGENTS.md 不存在**：提示用户"目标项目缺少 AGENTS.md 文件，请先创建"，中止操作
- **memory 规则或 yy-memory 技能已存在**：询问用户"目标项目已存在记忆系统，是覆盖还是更新？"，用户确认后继续
...
从 `templates/rule-template.md` 模板生成规则文件：

- 目标路径：`.agents/rules/memory/RULE.md`
- 如已存在，先读取后判断是覆盖还是追加
- 规则内容包含：触发场景、读取流程、写入流程、记忆系统架构、容量管理规则

### 步骤 5. 创建项目私有 yy-memory 技能

从 `templates/yy-memory-skill.md` 模板生成技能文件：

- 目标路径：`.agents/skills/yy-memory/SKILL.md`
- 如已存在，先读取后判断是覆盖还是追加
- 技能内容包含：描述、使用场景、指令步骤（commit 后执行记忆更新）
...
5. 创建 `work-log.md` 工作日志文件
6. 创建 `.memory/hubs/` 目录，从 `templates/hub-template.md` 生成 5 个主题 hub
7. 创建 `.memory/leaf/` 目录，从 `templates/leaf-template.md` 生成示例 leaf
...
- **不存在**：创建 `.gitignore`，添加 `.memory/` 条目
- **存在且未包含 `.memory/`**：在末尾追加 `.memory/` 条目
- **存在且已包含 `.memory/`**：跳过此步骤

### 步骤 9. 注入 AGENTS.md 规则引用
...
1. **创建结果**：操作类型（新建/更新）
2. **生成的目录结构**：使用树形结构展示
3. **记忆库路径**：`.memory/`（个人私有，已加入 .gitignore）
4. **触发方式**：对话启动时自动加载 MEMORY.md，手动执行 [yy-memory](./.agents/skills/yy-memory) 技能后更新记忆
5. **使用提醒**：建议用户如何验证安装成功
...
- `.memory/` 目录自动加入 `.gitignore`，确保个人记忆不提交到仓库

## 相关资源

- `templates/rule-template.md`：记忆管理规则模板
