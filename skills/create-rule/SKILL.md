---
name: create-rule
description: 在 .agents/rules 目录下创建或更新规则文档，并更新 AGENTS.md 中的引用关系
icon: 📝
parameters:
  - name: rule
...
这个技能用于在 `.agents/rules/` 目录下创建或更新规则文档，并更新 AGENTS.md 中的引用关系，确保 AI 助手能够理解项目的具体规范和最佳实践。
...
- 如果目录不存在，创建 `.agents/rules/` 目录并提示用户
...
列出 `.agents/rules/` 目录下所有文件。对每个现有规则文档，判断新内容是否属于其主题范围。
...
3. 在 Rules 章节中添加引用：`- .agents/rules/[文件名].md: [简要说明]`
...
→ 检查 .agents/rules/ 目录是否存在
→ 查找现有规则文档，没有找到合适的
→ 创建新文档 `.agents/rules/vue组件规范.md`
→ 在 AGENTS.md 中添加引用

**场景2：追加到现有规则文档**
用户说："/create-rule 所有 API 错误都要用统一的错误处理器，不能自己 catch 后静默忽略"
→ 检查 .agents/rules/ 目录
→ 找到 `.agents/rules/api调用最佳实践.md`
→ 在该文档中追加新内容
→ 不需要修改 AGENTS.md
...
→ 检查 .agents/rules/ 目录
→ 没有找到合适的现有文档
→ 创建新文档 `.agents/rules/移动端兼容性问题.md`
→ 在 AGENTS.md 中添加引用
...
- `.agents/rules/` 文件的内容深度 > AGENTS.md 的引用说明深度
