...
- **BEM 命名**：遵循 Block\_\_Element--Modifier 规范（全小写、横线连接）
- 嵌套不超过 3 层
...
- **CSS 兼容性指南**：`gap`、`aspect-ratio`、`100vh`、`inset`、`:has()` 等属性存在兼容性风险，需提供降级方案
- **不推荐**：`:has()` 伪类（Safari 15.4-15.6 存严重渲染 Bug）
...
- 空指针引用前检查对象（可选链 `?.` 或短路 `&&`）
- **⚠️ 可选链不推荐**：不推荐 `a?.b?.c`，建议使用 lodash `get(a, ['b', 'c'])` 替代
...
- 2 空格缩进，统一换行
- 嵌套不超过 3 层
- **不推荐**：CSS 嵌套原生写法（需经 PostCSS 编译后使用）
...
8. CSS 兼容性：`gap`、`aspect-ratio`、`100vh` 等属性需提供降级方案
