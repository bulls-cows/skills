...
修改完成后执行 `npm run lint`。

- 命令通过时，记录验证通过
- 缺少依赖时，补齐依赖清单并安装依赖后重试
- 配置错误时，修正本次新增或调整的配置后重试
- 既有代码存在 lint 或测试失败时，只修复由本次配置引入的误报，并列出既有问题
- 环境缺少 Python、Node.js 或 npm 时，停止验证并说明阻塞原因
...
- 格式：用简洁 Markdown 输出修改原因、影响范围、文件列表、流程覆盖、测试处理和验证结果
- 语气：中文、客观、面向执行
