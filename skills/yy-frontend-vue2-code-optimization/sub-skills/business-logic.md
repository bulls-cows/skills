...
读取 `.vue` 文件内容，理解其业务职责、数据流和交互关系，生成结构化业务说明，插入到 `<script>` 标签的最顶部。
...
- 关键方法的执行时序（如 `created → fetchData → computed 派生 → 用户操作触发`）
 ...
 * 改动内容: 优化 computed 优先策略 - 将 dataSource 过滤逻辑从 watch 移至 computed
 *
 * ---
 *
 * 改动时间: 2026-04-20 10:15:00
 * 改动内容: 新增导出功能，数据流向增加导出接口调用
 *
 * ---
 *
 * 改动时间: 2026-04-10 09:00:00
 * 改动内容: 生成首次业务逻辑说明
 *
 * ---
 *
 ...
 * @description 核心业务流程: created → 请求用户列表 → computed 派生分页数据 → 用户操作触发
 *
 * 数据来源:
 * - props: pageSize (分页大小，默认 20)
 * - API: apiGetUserList 接口获取用户列表数据
 * - data: searchQuery (查询条件)、loading (加载状态)、dataSource (列表数据)
 * - Vuex: user/module 提供用户权限信息
 *
 * 交互关系:
...
- **Vue2 特有**：需注明 Vuex store 模块（如有使用）
- **Options API 特有**：注明数据来自 `data()` 返回的对象属性
