...
1. **数据操作**：禁止连续解构数据（如 `...data.data`）；禁止父组件直接修改子组件数据；禁止多次修改 data 属性类型（后端给什么值用什么值，可新增属性但不允许修改原始数据类型）；禁止直接修改 props
2. **代码结构**：禁止使用 mixins；禁止多层 try/catch 嵌套
3. **封装原则**：逻辑简单时不要额外封装为 methods，直接在 template 中写内联表达式
...
1. **用户指定了文件或文件夹**：递归获取所有支持的文件类型（`.vue`、`.js`、`.css`、`.scss`、`.less`）
2. **用户未指定文件（默认行为）**：通过 `git diff --name-only HEAD` 和 `git diff --cached --name-only` 获取改动文件，合并去重后过滤
3. **终止条件**：无匹配文件时回复 **"当前没有需要优化的改动文件（支持 .vue、.js、.css、.scss、.less）。你可以指定文件或文件夹让我优化。"** 并终止执行
...
| 文件类型 | 优化要点 | 查阅参考 |
|----------|---------|----------|
| `.vue` | 脚本结构顺序、命名规范、代码风格、元素特性顺序、网络请求、computed 规范、组件传参、emit 事件、方法逻辑顺序、样式 BEM、注释规范 | [代码风格](./references/code-style.md)、[命名规范](./references/naming.md)、[组件开发规范](./references/component.md)、[注释规范](./references/comments.md)、[CSS 与 BEM 样式规范](./references/css-style.md) |
| `.js` | 代码风格、导入顺序（9 组）、命名规范、网络请求、注释 | [代码风格](./references/code-style.md)、[命名规范](./references/naming.md)、[注释规范](./references/comments.md) |
...
- [代码风格](./references/code-style.md)：基础格式规则、导入顺序（9 组）、等于运算符规则、性能优化
- [命名规范](./references/naming.md)：目录结构、API/事件/常量命名、Props 规范、事件白名单、布尔值命名
- [组件开发规范](./references/component.md)：脚本结构顺序、Script 顶部 JSDoc、元素特性顺序、网络请求、computed 规范、emit 事件、方法逻辑顺序、组件传参、拆分建议
