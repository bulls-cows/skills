...
| 风险等级 | 默认状态 | 包含任务 |
|---------|---------|---------|
| 🟢 零风险 | ✅ 自动勾选 | 业务逻辑梳理、文档注释增强 |
| 🟡 低/中风险 | ❌ 需确认 | 代码风格清洗（含 `==` 与 `===` 转换）、CSS/BEM 重构、语义化命名重构 |
...
- **scoped 样式同步**：进行 BEM 重构时，必须同步修改模板中的 `class` 属性，确保样式与模板一致
- **跨文件引用**：语义化命名重构涉及跨文件引用时，需提示用户影响范围并取得确认后再执行
- **== vs === 转换**：运算符转换属于逻辑变更，必须单独提醒用户确认，绝不自动执行
