...
| T03     | 代码风格清洗 | 🟡 中风险 | 导入排序(6组)、Options API 结构排序、模板属性顺序 | 文件子代理执行 |
| T04     | CSS/BEM 规范 | 🟡 中风险 | 类名转为 BEM 格式，scoped 同步修改                | 文件子代理执行 |
| T05     | 语义化命名   | 🟡 中风险 | API/事件/常量命名规范                             | 文件子代理执行 |
...
└─────────────────────────────────────────────────────────────────────┘
         ...
         ▼                              ▼
┌───────────────────────────┐  ┌─────────────────────────────────────┐
│  文件子代理（按文件并行）   │  │  高风险任务（主代理串行执行）        │
│  每个文件一个独立子代理     │  │  (每项改动需用户单独确认)            │
├───────────────────────────┤  ├─────────────────────────────────────┤
│ 文件子代理 A（FileA.vue）  │  │ T06 逻辑深度优化                     │
│ → 执行 T01, T02, T03,     │  │ T07 无效代码清理                     │
│   T04, T05                │  │ ⚠️ 逐项展示 diff → 用户确认 → 执行   │
├───────────────────────────┤  └─────────────────────────────────────┘
│ 文件子代理 B（FileB.js）   │
│ → 执行 T02, T03, T05      │
├───────────────────────────┤
...
3. 零风险任务执行（自动执行，无需确认）
   └── 按文件创建子代理，每个文件子代理执行 T01, T02，各文件并行

4. 中风险任务执行（用户确认后执行）
   ├── 用户选择要执行的任务（T03/T04/T05）
   └── 按文件创建子代理，每个文件子代理执行选中的中风险任务，各文件并行
...
**处理文件**: [filename]
**执行任务**: T01, T02, T03, T04, T05（按文件类型匹配）

**变更摘要**:

- ✅ [T01] 添加业务逻辑说明 JSDoc
- ✅ [T02] 增强模板/脚本/样式注释
- ✅ [T03] 导入排序、Options API 结构调整
- ⏭️ [T04] 无 BEM 相关改动
- ✅ [T05] 重命名 getUserInfo → apiGetUserInfo
