...
- 按用户确认的 ID 逐项调度执行。
- `.vue` 文件：业务逻辑梳理 → 代码风格清洗 → 文档注释 → CSS/BEM → 语义化命名 → 逻辑优化（确认后）
- `.js` 文件：代码风格清洗 → 文档注释 → 语义化命名 → 逻辑优化（确认后）
- `.css/.scss/.less` 文件：代码风格清洗 → CSS/BEM 架构规范
...
**风险点**：原代码可能使用不同响应结构；原有错误处理可能不同；`async/await` 改变执行时机。
...
**风险点**：响应式求值时机不同；带副作用的逻辑不能转为 computed；`try/catch` 在 computed 中行为与 methods 不同。
...
**风险点**：拆分后可能引入作用域/this 指向问题；内联表达式改变执行时机。
...
**风险点**：父组件监听的自定义事件名可能不在白名单中，改名或替换会导致父组件监听失效。
...
**风险点**：缺少 `default` 值可能导致 props 为 `undefined` 时运行报错；新增 type 声明可能触发类型异常。
...
1. **Vue 组件**：统一代码结构、规范命名、优化代码风格、规范网络请求、BEM 样式规范
2. **JavaScript**：统一导入顺序、规范命名、异步优化、移除冗余代码
