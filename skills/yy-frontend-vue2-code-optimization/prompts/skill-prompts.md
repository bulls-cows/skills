...
| T04 | [filename] | CSS/BEM 架构规范 | 类名转为 `block__element--modifier` 格式 | 🟡 低/中风险 |
| T05 | [filename] | 语义化命名重构 | `isXX` 前缀替换 / API 函数重命名 / 常量命名规范 | 🟡 低/中风险 |
...
| 🟡 低/中风险 | ❌ 未勾选 | 代码风格清洗（含 `==`/`===` 转换）、CSS/BEM 重构、语义化命名重构，需提示风险 |
...
以下各子技能按任务 ID 执行，详细规则请参考 `SKILL.md` 中的「详细优化规则」章节。
...
- 优先使用项目根目录 `.prettierrc` 配置；若不存在，参考 `assets/.prettierrc.json` 执行（仅供参考，不直接执行）
