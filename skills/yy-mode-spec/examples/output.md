...
> **文件关系说明**
>
> 本目录包含三个文件，以本文档（spec.md）为核心入口：
>
> | 文件 | 用途 |
> |------|------|
> | **spec.md**（← 当前文件） | 规格说明书，包含需求、技术设计和任务定义 |
> | **tasks.md** | 任务分解，按依赖顺序排列的实施任务，含状态标记 |
> | **checklist.md** | 验证清单；每项核对后立即更新复选框状态 |
>
> **中断续接**：如果任务完成一半中断，新会话应先读本文档了解全貌，然后查看 tasks.md 中未完成的任务继续实施。使用 checklist.md 时，每核对一项都必须立即更新对应复选框：通过标记为 `- [x]`，未通过保留 `- [ ]` 并记录未通过原因或下一步。
...
> 本文档是质量验证检查表，与 [spec.md](./spec.md)（规格说明书）、[tasks.md](./tasks.md)（任务分解）配合使用。每核对一项后必须立即更新对应复选框：通过标记为 `- [x]`，未通过保留 `- [ ]` 并记录未通过原因或下一步；所有任务完成且检查项均已通过后才可交付。
