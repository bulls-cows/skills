...
description: 审核 Vue3 项目 src 目录下代码改动，覆盖 9 大维度：组件规范、代码风格、命名、逻辑错误、网络请求、computed、安全、最佳实践、绝对禁止项。仅支持 <script setup> 组合式 API，不支持 React/Vue2。
---
...
| 4. 逻辑错误 | `references/logic-and-request.md` | 空指针、数组越界、逻辑判断、方法内部逻辑顺序、ref `.value` 访问 |
| 5. 网络请求规范 | `references/logic-and-request.md` | async/await + try/catch/finally、禁止多层 try/catch、禁止连续解构、统一响应模式 |
| 6. computed 规范 | `references/logic-and-request.md` | try/catch 包裹、有意义命名 |
| 7. 安全漏洞 | `references/best-practice.md` | XSS 风险（v-html）、敏感信息泄露、硬编码密钥/Token/密码 |
| 8. 最佳实践 | `references/best-practice.md` | 调试代码清理、BEM + scoped、未使用变量、defineExpose、组件拆分、组件懒加载、KeepAlive |
| 9. 绝对禁止项 | `references/best-practice.md` | 连续解构、修改子组件数据、修改 ref/reactive 类型、修改 props、this、Options API、mixins、多层 try/catch |
