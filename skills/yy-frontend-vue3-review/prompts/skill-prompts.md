...
- **ref/computed 使用**：优先使用 `ref`，复杂对象使用 `reactive`；除后端交互数据和部分定时器外，其它尽可能使用 `computed`；ref 访问必须使用 `.value`。
...
- **Hooks 规范**：可复用逻辑超过 30 行或跨 2 个以上组件时，必须抽离为 Hook；Hooks 必须返回对象（推荐 `toRefs` 解构后返回），禁止直接返回 reactive 对象；禁止将 Hooks 挂载到响应式数据上。
