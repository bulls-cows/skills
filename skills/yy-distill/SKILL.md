...
- 按能力模型重构内容时，只能重构用户明确指定的范围；若涉及更新现有内容，必须先执行确认流程，再进行修改。
...
- **更新现有内容**：用户要求修改已有文件、已有文档、已有技能、已有规则或其他既存内容，必须执行确认流程
- **新建内容或仅输出草案**：可直接生成重构草案，但仍需显式说明改写依据和影响范围

#### 5.3 更新确认流程

命中更新场景时，执行以下确认流程，未确认前不得进行修改：

- **方向不明确时**：先展示方案方向，至少包含目标、方法、涉及范围，等用户确认方向正确后，再展示计划
- **方向明确时**：直接展示计划
- 展示计划后，必须等待用户确认，收到确认后才可执行重构或更新操作
...
- 检查更新场景是否已完成确认流程；未确认则只输出计划，不执行修改
