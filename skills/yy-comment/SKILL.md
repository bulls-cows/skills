...
| 场景 | 是否需要注释 | 说明 |
|------|-------------|------|
| 业务规则/领域逻辑 | ✅ 必须 | 涉及特定业务知识，非开发者难以理解 |
| 复杂算法 | ✅ 必须 | 阅读时间 > 30 秒的逻辑 |
| 非直观的边界处理 | ✅ 必须 | 如特殊的数据转换、兼容性处理 |
| 纯技术实现 | ⚠️ 视情况 | 通用技术模式（如循环、映射）不需要 |
...
### 修改文件
- `src/utils/format.ts` - 添加 3 个函数的 JSDoc 注释
