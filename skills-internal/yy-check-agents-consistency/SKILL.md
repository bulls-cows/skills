...
description: >
  检查 AGENTS.md 中的通用章节与 yy-create-agents 生成模板的一致性。当路径格式规范或终端命令能力识别发生改动时使用，确保 yy-create-agents 后续生成的 AGENTS.md 同步包含相同内容。
...
- **章节顺序不一致**：调整为 `路径格式规范`、`终端命令能力识别`、`需要遵守的规则` 的顺序
- **无不一致问题**：不修改文件
