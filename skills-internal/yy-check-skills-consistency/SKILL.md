...
说明同步时，`SKILL.md` 的 frontmatter `description` 是技能用途的语义权威来源，但不是 README 说明列的逐字覆盖模板。README 技能列表主要给人看，说明列只应介绍技能是什么、解决什么问题，并且必须是精简归纳后的表达，不应直接照搬原文，也不应混入触发场景、排除条件或实现细节。
