...
当技能名发生变更或技能被删除时，需要将旧技能名同步到 `scripts/install-skills.ts` 的 `oldSkillNamesToDelete` 数组中。该数组的作用是：执行 `npm run install:skills` 时，脚本会先将当前 skills/ 目录中的技能与该数组合并，再统一调用 `skills remove -g` 卸载全局技能，最后重新安装。如果不将旧技能名加入此数组，已安装到各 agent 全局目录中的旧技能残留文件不会被清除。

**信息来源**：

- **git 本地改动**：通过 `git diff` 或 `git status` 检测 skills/ 目录中已删除或已重命名的目录
- **AI 聊天上下文**：当前对话中用户明确告知的技能名变更或删除信息
